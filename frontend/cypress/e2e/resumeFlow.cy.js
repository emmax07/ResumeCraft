...
describe('Full User Journey Test: Signup, Login, Resume Creation, Logout', () => {
    it('should signup, login, fill resume, preview, download, and logout successfully', () => {
      ...
      cy.get('textarea[name="profile_summary"]').type('Passionate Software Developer');
      cy.get('textarea[name="key_skills"]').type('JavaScript, React, Node.js');
  
      cy.get('textarea[name="workExperience-0"]').type('Software Engineer at XYZ Corp');
      cy.get('textarea[name="Education-0"]').type('Bachelor of Technology in Computer Science');
      cy.get('textarea[name="Certifications-0"]').type('AWS Certified Solutions Architect');
      cy.get('textarea[name="Projects-0"]').type('Resume Builder Application');
      cy.get('textarea[name="Research Publications-0"]').type('Paper on Cloud Computing 2024');
